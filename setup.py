...
class build_ext_options:
    def build_options(self):
        src_dir = os.path.join(os.path.dirname(__file__), 'thinc_gpu_ops', '_files')
...
def setup_package():
    ...
    with chdir(root):
        with open(os.path.join(root, 'thinc_gpu_ops', 'about.py')) as f:
            about = {}
            exec(f.read(), about)
        ...
        if CUDA is None:
            ...
            with chdir(root):
                ext_modules.append(
                    Extension("thinc_gpu_ops.gpu_ops",
                        sources=["thinc_gpu_ops/gpu_ops.cpp", "include/_cuda_shim.cu"],
        ...
        if not is_source_release(root):
            generate_cython(root, 'thinc_gpu_ops')
